...
class ContinuousLVLM(nn.Module):
    ...
    def forward(
        ...
        if image_embeds is not None and image_embeds_cmp.shape[0] > 0:
            image_embeds_lm = self.input_resampler(
                image_embeds_cmp
            ...
            image_embeds_cmp_fake = torch.randn(
                1, self.output_resampler.num_queries, self.output_resampler.embed_dim
            ).to(input_embeds.device, dtype=input_embeds.dtype)

            image_embeds_lm = self.input_resampler(image_embeds_cmp_fake)
            if self.add_patch_pos:
                rel_pos_embed = self.patch_pos_embed.mean(0, keepdim=True).unsqueeze(
                    1
                )  # 1, 1, dim
                image_embeds_lm = image_embeds_lm + rel_pos_embed
        ...
        if has_image_input:
            ...
            input_embeds[:1, : self.input_resampler.num_queries, :] += (
                0.0 * image_embeds_lm[:1, :, :]
            )
        ...
        if has_image_output:
            target_embeds = image_embeds[
                embeds_gen_mask
            ]  # num_imgs_gen_target x nq_in x dim_in, 2 x 256 x 4096

            if self.vit_down:
                target_embeds = target_embeds.permute(0, 2, 1)  # NLD -> NDL
                target_embeds = F.avg_pool1d(
                    target_embeds, kernel_size=self.pool_size, stride=self.stride
                )
            ...
            output_image_embeds = last_hidden_state[ids_gen_mask].view(
                num_imgs_for_rec, -1, dim
            )  # 128 x 4096 -> 2 x 64 x 4096

            recon_image_embeds = self.output_resampler(
                output_image_embeds
            )  # 2 x 256 x 4096
            ...
            if self.mse:
                ...
                rec_loss = F.mse_loss(
                    recon_image_embeds, target_embeds.detach()
                )  # for zero3 compatibility
            else:
            ...
            output_image_embeds = (
                torch.randn(
                    1, self.input_resampler.num_queries, self.input_resampler.embed_dim
                ).to(input_embeds.device, dtype=input_embeds.dtype)
                + 0.0 * last_hidden_state[0, : self.input_resampler.num_queries, :]
            )
    ...
    def generate(
        ...
        if logits_processor is None:
            ...
            logits_processor.append(
                AutoImageTokenGenerationProcessor(
                    tokenizer=tokenizer, num_img_gen_tokens=num_img_gen_tokens
                )
            )
        ...
        last_hidden_states = torch.cat(
            [hidden_state[-1] for hidden_state in output.hidden_states], dim=1
        )[0, input_ids.shape[1] :, :]
