...
from .pipeline_stable_diffusion_xl_t2i_edit import (
...
class SDXLAdapter(nn.Module):
    ...
    def forward(
        self, noisy_latents, timesteps, image_embeds, text_embeds, noise, time_ids
    ):
        image_embeds, pooled_image_embeds = self.resampler(image_embeds)

        unet_added_conditions = {
            "time_ids": time_ids,
            "text_embeds": pooled_image_embeds,
        }
    ...
    def get_image_embeds(
        ...
        assert (
            ...
            + int(image_tensor is not None)
            + int(image_embeds is not None)
            == 1
        )
        ...
        if image_tensor is not None:
            ...
            image_tensor_neg = torch.zeros(1, 3, image_size, image_size).to(
            ...
            if self.vit_down:
                ...
                image_embeds_neg = F.avg_pool1d(
                    image_embeds_neg, kernel_size=4, stride=4
                )
    ...
    def generate(
        ...
        **kwargs,
        ...
        (
            ...
            pooled_uncond_image_prompt_embeds,
        ) = self.get_image_embeds(
            image_pil=image_pil,
...
class SDXLAdapterWithLatentImage(SDXLAdapter):
    def __init__(
        self, unet, resampler, full_ft=False, set_trainable_late=False, vit_down=False
    ) -> None:
    ...
    def from_pretrained(
        ...
        **kwargs,
        ...
        model = cls(
            unet=unet,
            resampler=resampler,
            set_trainable_late=set_trainable_late,
            **kwargs,
        )
        if pretrained_model_path is not None:
            if not os.path.isfile(pretrained_model_path):
