...
adapter = (
    hydra.utils.instantiate(adapter_cfg, unet=unet).to(device_2, dtype=dtype).eval()
)
...
output = agent_model.generate(
    tokenizer=tokenizer, input_ids=input_ids, num_img_gen_tokens=num_img_out_tokens
)
...
if output["has_img_output"]:
    images = adapter.generate(
        image_embeds=output["img_gen_feat"].to(device_2), num_inference_steps=50
    )
