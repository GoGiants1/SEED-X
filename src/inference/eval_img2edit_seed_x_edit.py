...
print("init vae")
vae = AutoencoderKL.from_pretrained(diffusion_model_path, subfolder="vae").to(
    device, dtype=dtype
)
print("init unet")
unet = UNet2DConditionModel.from_pretrained(diffusion_model_path, subfolder="unet").to(
    device, dtype=dtype
)
...
embeds_cmp_mask = torch.tensor([True] * image_tensor.shape[0]).to(
...
for _ in range(patch_length - 1):
    image_tokens += (
        BOP_TOKEN
        + "".join(IMG_TOKEN.format(int(item)) for item in range(num_img_in_tokens))
        + EOP_TOKEN
    )
image_tokens += (
    BOI_TOKEN
...
boi_indices = torch.where(
    torch.logical_or(input_ids == boi_token_id, input_ids == bop_token_id)
)[0].tolist()
eoi_indices = torch.where(
    torch.logical_or(input_ids == eoi_token_id, input_ids == eop_token_id)
)[0].tolist()
...
if output["has_img_output"]:
    ...
    save_path = os.path.join(
        save_dir, str(len(os.listdir(save_dir))) + "_" + instruction + ".jpg"
    )
